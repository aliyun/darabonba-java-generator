...
function shouldChange(pomVersion, importVersion) {
  ...
  try {
    ...
    var teargetVersion = importVersion.split('.');
    if (parseInt(sourceVersion[0]) > parseInt(teargetVersion[0])) {
      return false;
    }
    if (parseInt(sourceVersion[1]) > parseInt(teargetVersion[1])) {
      return false;
    }
    if (parseInt(sourceVersion[2]) > parseInt(teargetVersion[2])) {
      return false;
    }
  } catch (error) {
    // empty
...
class Visitor {
  ...
  eachImport(imports) {
    ...
    for (let i = 0; i < imports.length; i++) {
      ...
      const releaseJava = teaFile.releases.java;
      const javaPkg = teaFile.java;
      if (!javaPkg) {
  ...
  visitStmt(ast, level) {
    ...
    if (ast.type === 'return') {
      ...
      this.visitRequestAssign(ast, level);
      ...
      this.visitIfRequestAssign(ast, level);
  ...
  visitExpr(ast, level) {
    if (ast.type === 'boolean') {
      ...
      this.visitModuleCall(ast, level);
    } else if (ast.type === 'array') {
  ...
  visitModuleCall(ast, level) {
    assert.equal(ast.type, 'module_call');
    this.emit('await ', level);
    this.emit(`${_name(ast.id)}.`);
    for (let i = 0; i < ast.propertyPath.length; i++) {
      const prop = ast.propertyPath[i];
      this.emit(`${_name(prop)}`);
    }
    this.visitArgs(ast.args, level);
  }
  ...
  visitConstructModelField(ast, level, variableName) {
    for (var i = 0; i < ast.fields.length; i++) {
      if (i > 0) {
        this.emit(';\n');
      }
      const item = ast.fields[i];
      this.emit(`${variableName}.${item.fieldName.lexeme} = `, level);
      this.visitExpr(item.expr, level);
    }
  }
  ...
  visitIfRequestAssign(ast, level) {
    assert.equal(ast.type, 'ifRequestAssign');
    this.emit('if (', level);
    this.visitExpr(ast.condition, level + 1);
    this.emit(') {\n');
    for (let i = 0; i < ast.assigns.length; i++) {
      this.visitRequestAssign(ast.assigns[i], level + 1);
    }
    if (ast.elseIfs) {
      for (let i = 0; i < ast.elseIfs.length; i++) {
        let elseIf = ast.elseIfs[i];
        this.emit('} else if (', level);
        this.visitExpr(elseIf.condition, level + 1);
        this.emit(') {\n');
        for (let i = 0; i < elseIf.assigns.length; i++) {
          this.visitRequestAssign(elseIf.assigns[i], level + 1);
        }
      }
    }
    if (ast.elseAssigns) {
      ...
      for (let i = 0; i < ast.elseAssigns.length; i++) {
        this.visitRequestAssign(ast.elseAssigns[i], level + 1);
      }
    }
    this.emit('}', level);
    this.emit('\n');
  ...
  visitRequestAssign(ast, level) {
    ...
    if (ast.left.type === 'id') {
      ...
      this.emit(`.${_lowerFirst(name)}.put(`);
      for (var i = 0; i < ast.left.propertyPath.length; i++) {
        this.emit(`"${_name(ast.left.propertyPath[i])}", `);
