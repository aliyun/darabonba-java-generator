...
class Visitor {
  ...
  async visitPom() {
    ...
    if (!havePom) {
      ...
      dependenciesClass.push('com.aliyun:tea:[1.0.6, 2.0.0)');
  ...
  visitBuildMethod(ast, level) {
    ...
    this.emit(`public static ${className} build(java.util.Map<String, ?> map) {\n`, level);
  ...
  eachFunction(ast, level) {
    ...
    if (_name(ast.functionName) === 'main') {
      this.emit('(String[] args) throws Exception');
    } else {
