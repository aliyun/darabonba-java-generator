...
class Visitor {
  ...
  visitAnnotation(annotation, level) {
    ...
    if (descriptionText !== '') {
      ...
      const descriptionTexts = Annotation.md2Html(descriptionText).trimEnd();
    ...
    if (summaryText !== '') {
      ...
      const summaryTexts = Annotation.md2Html(summaryText).trimEnd();
      summaryTexts.split('\n').forEach((line) => {
        this.emit(` * ${line}\n`, level);
      });
      hasNextSection = true;
    ...
    if (params.length > 0) {
      ...
      params.forEach((item) => {
        this.emit(` * @params ${item.name} ${item.text}\n`, level);
      });
      hasNextSection = true;
    }
    if (returnText !== '') {
  ...
  visitModelBody(ast, level, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (description || example || typeof checkBlank !== 'undefined' || typeof nullable !== 'undefined' || typeof sensitive !== 'undefined') {
        this.emit('/**\n', level);
        if (description) {
          const descriptions = Annotation.md2Html(description).trimEnd().split('\n');
        ...
        if (example) {
          ...
          const examples = Annotation.md2Html(example).trimEnd().split('\n');
