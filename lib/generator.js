...
const RESPONSE_ = 'response_';
const RUNTIME_ = 'runtime_';
const {
  _name,
  _type,
  _lowerFirst,
...
class Visitor {
  ...
  visitWhile(ast, level, options) {
    assert.equal(ast.type, 'while');
    this.emit('\n');
    this.emit('while (', level);
    this.visitExpr(ast.condition, level + 1, options);
    this.emit(') {\n');
    this.visitStmts(ast.stmts, level + 1, options);
    this.emit('}\n', level);
  }

  visitFor(ast, level, options) {
    assert.equal(ast.type, 'for');
    this.emit(`for (`, level);
    this.visitType(ast.list.inferred.itemType, undefined, true);
    this.emit(` ${_name(ast.id)} : `);
    this.visitExpr(ast.list, level + 1, options);
    this.emit(') {\n');
    this.visitStmts(ast.stmts, level + 1, options);
    this.emit('}\n', level);
  }
  ...
  visitModule(ast, level) {
    ...
    for (let i = 0; i < exceptions.length; i++) {
      const exceptionName = exceptions[i].exceptionName.lexeme;
      this.exceptionBefore(level, exceptionName, exceptions[i].extendOn);
      this.eachException(exceptions[i], level);
      this.emit('}\n', level);
      this.save('exceptions/' + exceptionName + '.java');
    }
    ...
    // Add exception import if there are exceptions
    if (exceptions.length > 0) {
      extendParam.writeExceptionImport = true;
    }
  ...
  eachImport(imports, usedModels) {
    ...
    for (let i = 0; i < imports.length; i++) {
      ...
      let targetPath = '';
      if (moduleDir && (moduleDir.startsWith('./') || moduleDir.startsWith('../'))) {
        targetPath = path.join(this.config.pkgDir, moduleDir);
      } else if (moduleDir && moduleDir.startsWith('/')) {
        targetPath = moduleDir;
      } else if (lock[moduleDir]) {
        targetPath = path.join(this.config.pkgDir, lock[moduleDir]);
      } else {
        throw new Error(`Module '${aliasId}' not found in libraries configuration`);
      }
  ...
  visitType(ast, modelName, variable) {
    if (ast.type === 'map') {
      ...
      if (ast.moduleName) {
        ...
        if (this.conflictModels.get(modelMap)) {
          // Check if this is an exception (by checking usedExternException)
          let isException = false;
          if (this.usedExternException) {
            const usedEx = this.usedExternException.get(ast.moduleName);
            if (usedEx && usedEx.has(_name(ast))) {
              isException = true;
            }
          }
          this.emit(`${this.imports[ast.moduleName].package}.${isException ? 'exceptions.' : 'models.'}`);
        }
      } else {
      ...
      for (let i = 0; i < ast.path.length; i++) {
        ...
        if (i > 0) {
          ...
          innerModelName = item.lexeme;
        }
        pathName += item.lexeme;
      }
      let subModelName = _subModelName(pathName, this.conflictModelNameMap, this.allModleNameMap);
      if (modelName === innerModelName) {
        this.emit(subModelName);
      } else {
        const modelMap = `${innerModelName}:${pathName}`;
        if (this.conflictModels.get(modelMap)) {
          // Check if this is an exception (by checking usedExternException)
          let isException = false;
          if (this.usedExternException) {
            const usedEx = this.usedExternException.get(innerModelName);
            if (usedEx && usedEx.has(pathName)) {
              isException = true;
            }
          }
          this.emit(`${this.imports[innerModelName].package}.${isException ? 'exceptions.' : 'models.'}`);
        }
        this.emit(innerModelName + '.' + subModelName);
      }
      ...
      var innerModelName = rest[0].lexeme;
      var moduleName = moduleId.lexeme;
      const checkModel = moduleName + ':' + pathName;
      
      // Check if this is an exception (by checking if the innerModelName is an exception)
      const isException = this.exceptions && this.exceptions.has(innerModelName);
      
      if (this.conflictModels.get(checkModel)) {
        var packageName = `${this.imports[moduleName].package}.${isException ? 'exceptions.' : 'models.'}`;
        this.emit(packageName + innerModelName + subModelName);
      } else {
        if (isException) {
          this.emit(`${this.imports[moduleName].package}.exceptions.${innerModelName}${subModelName}`);
        } else {
          this.emit(innerModelName + subModelName);
        }
      }
      ...
      this.emit(_type(ast.name, variable));
      ...
      // Check if this is a module.exception reference
      if (ast.moduleName && ast.name) {
        // This is a module.exception reference like Source.Err3
        const importInfo = this.imports[ast.moduleName];
        if (importInfo) {
          this.emit(`${importInfo.package}.exceptions.${ast.name}`);
        } else {
          this.emit(_type(ast.lexeme || ast.name, variable));
        }
      } else {
        // This is a regular module reference
        const importInfo = this.imports[ast.lexeme];
        if (importInfo) {
          let className = importInfo.className || 'Client';
          this.emit(`${importInfo.package}.${className}`);
        } else {
          this.emit(_type(ast.lexeme || ast.name, variable));
        }
      }
      ...
      let type = 'models';
      if (this.predefined && this.predefined[ast.lexeme] && this.predefined[ast.lexeme].isException) {
        type = 'exceptions';
      }
      const modelMap = `${_name(ast)}`;
      if (this.conflictModels.get(modelMap)) {
        this.emit(`${this.package}.${type}.`);
      }
      this.emit(modelMap);
    } else if (ast.idType === 'exception') {
      const exceptionMap = `${_name(ast)}`;
      if (ast.moduleName) {
        this.emit(`${this.imports[ast.moduleName].package}.exceptions.`);
      } else {
        this.emit(`${this.package}.exceptions.`);
      }
      this.emit(exceptionMap);
  ...
  visitAPIBody(ast, level) {
    ...
    this.emit(`TeaRequest ${REQUEST_} = new TeaRequest();\n`, level);
    if (ast.stmts.stmts) {
  ...
  visitRuntimeBefore(ast, level) {
    ...
    this.emit(`java.util.Map<String, Object> ${RUNTIME_} = `, level);
    ...
    this.emit(`while (Tea.allowRetry((java.util.Map<String, Object>) ${RUNTIME_}.get("retry"), _retryTimes, _now)) {\n`, level);
    this.emit('if (_retryTimes > 0) {\n', level + 1);
    this.emit(`int backoffTime = Tea.getBackoffTime(${RUNTIME_}.get("backoff"), _retryTimes);\n`, level + 2);
  ...
  visitStmt(ast, level, options) {
    let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    this.visitComments(comments, level);
    if (ast.type === 'return') {
      this.visitReturn(ast, level, options);
    } else if (ast.type === 'if') {
      this.visitIf(ast, level, options);
    } else if (ast.type === 'throw') {
      this.visitThrow(ast, level);
    } else if (ast.type === 'assign') {
      this.visitAssign(ast, level, options);
    } else if (ast.type === 'retry') {
      this.visitRetry(ast, level);
    } else if (ast.type === 'break') {
      this.emit(`break;\n`, level);
    } else if (ast.type === 'declare') {
      this.visitDeclare(ast, level, options);
    } else if (ast.type === 'while') {
      this.visitWhile(ast, level, options);
    } else if (ast.type === 'for') {
      this.visitFor(ast, level, options);
    } else if (ast.type === 'try') {
      this.visitTry(ast, level, options);
    } else {
      this.emit(``, level);
      this.visitExpr(ast, level, options);
      this.emit(';\n');
    }
  }

  visitTry(ast, level, options) {
    this.emit('try {\n', level);
    this.visitStmts(ast.tryBlock, level + 1, options);
    this.emit('}', level);
    
    // Handle multiple catch blocks (new syntax)
    if (ast.catchBlocks && ast.catchBlocks.length > 0) {
      ast.catchBlocks.forEach(catchBlock => {
        ...
        if (!catchBlock.id.type) {
          // Generic catch block without specific type
          this.emit(` catch (TeaException ${_name(catchBlock.id)}) {\n`);
        } else {
        ...
        this.visitStmts(catchBlock.catchStmts, level + 1, options);
        this.emit('}', level);
      });
    } else if (ast.catchBlock) {
      // Handle single catch block (legacy syntax)
      let errorName = _name(ast.catchId);
      if (ast.catchType) {
        this.emit(` catch (`);
        this.visitType(ast.catchType);
        this.emit(` ${errorName}) {\n`);
      } else {
        this.emit(` catch (TeaException ${errorName}) {\n`);
      }
      this.visitStmts(ast.catchBlock, level + 1, options);
      this.emit('}', level);
    }

    if (ast.finallyBlock) {
      this.emit(' finally {\n');
      this.visitStmts(ast.finallyBlock, level + 1, options);
      this.emit('}', level);
    }
    this.emit('\n', level);
  }
  ...
  visitFieldType(value, node, modelName) {
    if (value.fieldType === 'array') {
      ...
      if (value.fieldItemType.tag === 8) {
        ...
        || value.fieldItemType.type === 'subModel'
        || value.fieldItemType.type === 'moduleModel') {
        this.visitType(value.fieldItemType, modelName);
      ...
      this.emit(`java.util.Map<${_type(value.keyType.lexeme)}, `);
      if (value.valueType.type) {
        this.visitType(value.valueType, modelName);
      } else {
      ...
      if (value.fieldType.idType && value.fieldType.idType === 'module') {
        ...
        this.visitType(value.fieldType, modelName);
      } else {
  ...
  visitObjectField(ast, level) {
    ...
    if (ast.type === 'objectField') {
      var key = ast.fieldName.lexeme || ast.fieldName.string;
  ...
  visitCall(ast, level, options) {
    assert.equal(ast.type, 'call');
    if (ast.left.type === 'method_call') {
      this.visitMethodCall(ast, level, options);
    } else if (ast.left.type === 'instance_call') {
      this.visitInstanceCall(ast, level, options);
    } else if (ast.left.type === 'static_call') {
      this.visitStaticCall(ast, level, options);
  ...
  visitStaticCall(ast, level, options) {
    assert.equal(ast.left.type, 'static_call');
    if (ast.left.id.type === 'builtin_module') {
      this.visitBuiltinStaticCall(ast, level, options);
      return;
    }
    ...
    for (let i = 0; i < ast.args.length; i++) {
      ...
      this.visitExpr(expr, level, options);
      if (expr.needCast) {
        this.emit(')');
      }
  ...
  visitBuiltinStaticCall(ast, level, options) {
    ...
    if (!builtiner[func]) {
      throw new Error(`Function ${func} not found in module ${moduleName}`);
    }
    builtiner[func](ast, level, options);
  }


  visitInstanceCall(ast, level, options) {
    ...
    if (ast.builtinModule && this.builtin[ast.builtinModule] && this.builtin[ast.builtinModule][method]) {
      this.builtin[ast.builtinModule][method](ast, level, options);
    } else {
  ...
  visitMethodCall(ast, level, options) {
    ...
    if (ast.isStatic) {
      ...
      if (wrapName.startsWith('$') && this.builtin[wrapName]) {
        ...
        if (!this.builtin[wrapName][method]) {
          throw new Error(`Method ${method} not found in ${wrapName}`);
        }
        this.builtin[wrapName][method](ast, level, options);
        return;
  ...
  visitExpr(ast, level, options) {
    if (ast.type === 'boolean') {
      ...
      if (ast.elements.length === 3 && ast.elements[0].type === 'element' && ast.elements[2].type === 'element' &&
        ast.elements[1].type === 'expr' && ast.elements[1].expr.type === 'call') {
        var callItem = ast.elements[1];
        if (callItem.expr.type === 'property_access' && _name(callItem.expr.id) === '__module') {
          let value = this.__module;
          for (let i = 0; i < callItem.expr.propertyPath.length; i++) {
            value = value[_name(callItem.expr.propertyPath[i])];
          }
          this.emit('"');
          this.emit(value);
          this.emit('"');
        } else {
          this.visitExpr(callItem.expr, level, options);
          this.emit('.toString()');
        }
      } else {
        for (let i = 0; i < ast.elements.length; i++) {
          ...
          if (item.type === 'element') {
            ...
            if (item.expr.type === 'property_access' && _name(item.expr.id) === '__module') {
              ...
              this.visitExpr(item.expr, level, options);
            }
          } else {
            throw new Error('unimpelemented');
          }
      ...
      this.visitCall(ast, level, options);
    } else if (ast.type === 'construct') {
      this.visitConstruct(ast, level);
    } else if (ast.type === 'array') {
      this.visitArray(ast, level, options);
    } else if (ast.type === 'and') {
      this.visitExpr(ast.left, level, options);
      this.emit(' && ');
      this.visitExpr(ast.right, level, options);
    } else if (ast.type === 'or') {
      this.visitExpr(ast.left, level, options);
      this.emit(' || ');
      this.visitExpr(ast.right, level, options);
      ...
      this.emit('!');
      this.visitExpr(ast.expr, level, options);
      ...
      if (ast.args) {
        for (let i = 0; i < ast.args.length; i++) {
          ...
          this.visitExpr(ast.args[i], level, options);
        }
      }
      this.emit(')');
    } else if (ast.type === 'map_access') {
      this.visitMapAccess(ast, true, options);
    } else if (ast.type === 'array_access') {
      this.visitArrayAccess(ast, true, level, options);
    } else {
      throw new Error('unimpelemented');
    }
  }

  visitMapAccess(ast, isExpr, level, options) {
    ...
    if (isExpr) {
      this.emit(`${expr}.get(`);
      this.visitExpr(ast.accessKey, level, options);
      this.emit(`)`);
    } else {
  ...
  visitArrayAccess(ast, isExpr, level, options = {}) {
    ...
    if (isExpr) {
      if (options.main && options.argsName === expr) {
        this.emit(`${expr}[`);
  ...
  visitConstruct(ast, level) {
    ...
    // Handle exception construction
    if (ast.aliasId && ast.aliasId.lexeme && ast.aliasId.idType === 'exception') {
      const exceptionName = ast.aliasId.lexeme;
      if (ast.aliasId.moduleName) {
        this.emit(`${this.imports[ast.aliasId.moduleName].package}.exceptions.${exceptionName}`);
      } else {
        this.emit(`${this.package}.exceptions.${exceptionName}`);
      }
    } else if (ast.aliasId && ast.aliasId.lexeme) {
      // Handle regular module construction
      const moduleName = ast.aliasId.moduleName || ast.aliasId.lexeme;
      const importInfo = this.imports[moduleName];
      if (importInfo) {
        let className = importInfo.className || 'Client';
        this.emit(`${importInfo.package}.${className}`);
      } else {
    ...
    } else if (ast.inferred && ast.inferred.name) {
      const importInfo = this.imports[ast.inferred.name];
      if (importInfo) {
        let className = importInfo.className || 'Client';
        let pathName = importInfo.package;
        this.emit(`${pathName}.${className}`);
      } else if (ast.inferred.type === 'exception') {
        // Handle exception construction
        if (ast.inferred.moduleName) {
          this.emit(`${this.imports[ast.inferred.moduleName].package}.exceptions.${ast.inferred.name}`);
        } else {
          this.emit(`${this.package}.exceptions.${ast.inferred.name}`);
        }
      } else if (ast.inferred.idType === 'exception') {
        // Handle exception construction
        if (ast.inferred.moduleName) {
          this.emit(`${this.imports[ast.inferred.moduleName].package}.exceptions.${ast.inferred.name}`);
        } else {
  ...
  visitArray(ast, level, options) {
    ...
    for (let i = 0; i < ast.items.length; i++) {
      ...
      this.visitComments(comments, level + 1);
      this.emit('', level + 1);
      this.visitExpr(item, level + 1, options);
  ...
  visitConstructModel(ast, level) {
    ...
    if (ast.inferred) {
      this.visitType(ast.inferred);
    } else if (ast.inferredType) {
      this.visitType(ast.inferredType);
    } else if (ast.aliasId && ast.aliasId.lexeme) {
      // Handle exception construction by aliasId
      if (ast.aliasId.idType === 'exception') {
        const exceptionName = ast.aliasId.lexeme;
        if (ast.aliasId.moduleName) {
          this.emit(`${this.imports[ast.aliasId.moduleName].package}.exceptions.${exceptionName}`);
        } else {
      ...
      } else if (ast.aliasId.lexeme === '$Error') {
        // Handle builtin $Error exception
        this.emit('DaraException');
      } else if (ast.aliasId.moduleName && ast.aliasId.lexeme) {
        // Handle module.exception format like Source.Err3 where moduleName="Source", lexeme="Err3"
        const moduleName = ast.aliasId.moduleName;
        const exceptionName = ast.aliasId.lexeme;
        if (this.imports[moduleName]) {
          this.emit(`${this.imports[moduleName].package}.exceptions.${exceptionName}`);
        } else {
          this.emit(ast.aliasId.lexeme);
        }
      } else if (ast.aliasId.lexeme.includes('.')) {
        // Handle module.exception format like "Source.Err3"
        const parts = ast.aliasId.lexeme.split('.');
        const moduleName = parts[0];
        const exceptionName = parts[1];
        if (this.imports[moduleName]) {
          this.emit(`${this.imports[moduleName].package}.exceptions.${exceptionName}`);
        } else {
          this.emit(ast.aliasId.lexeme);
      ...
      // Handle exception construction without inferred type
      this.emit('TeaException');
    }
    this.emit(`()`);
  ...
  visitIf(ast, level, options) {
    ...
    if (ast.elseIfs) {
      for (let i = 0; i < ast.elseIfs.length; i++) {
        ...
        this.emit(' else if (');
        this.visitExpr(branch.condition, level + 1, options);
        this.emit(') {\n');
        this.visitStmts(branch.stmts, level + 1, options);
        this.emit('}', level);
      }
    }
    ...
    if (ast.elseStmts) {
      ...
      for (let i = 0; i < ast.elseStmts.stmts.length; i++) {
        this.visitStmt(ast.elseStmts.stmts[i], level + 1, options);
      }
      this.emit('}', level);
    }
  ...
  visitThrow(ast, level) {
    if (ast.expr.type === 'construct_model') {
      ...
      // Handle exception construction
      this.emit('throw ', level);
      this.visitConstruct(ast.expr, level);
      this.emit(';\n');
    } else {
  ...
  visitAssign(ast, level, options = {}) {
    ...
    if (ast.left.type === 'id') {
      ...
      if (!(options.main && options.argsName === _name(ast.left.id))) {
        isCollection = true;
      }
      this.visitArrayAccess(ast.left, false, level, options);
    } else {
      throw new Error('unimpelemented');
    }
    if (!isCollection) {
  ...
  visitDeclare(ast, level, options) {
    var id = _name(ast.id);
    this.emit(``, level);
    this.visitType(ast.expr.inferred, undefined, true);
    this.emit(` ${id} = `);
    this.visitExpr(ast.expr, level, options);
    this.emit(';\n');
  }
  ...
  visitStmts(ast, level, options) {
    assert.equal(ast.type, 'stmts');
    let node;
    for (var i = 0; i < ast.stmts.length; i++) {
      node = ast.stmts[i];
      this.visitStmt(node, level, options);
    }
    if (node) {
      //find the last node's back comment
  ...
  visitFunctionBody(ast, level, options) {
    assert.equal(ast.type, 'functionBody');
    this.visitStmts(ast.stmts, level, options);
  }
  ...
  eachFunction(ast, level) {
    ...
    if (_name(ast.functionName) === 'main') {
      ...
      this.emit(`(String[] ${_name(ast.params.params[0].paramName)})`);
    } else {
    ...
    if (ast.functionBody) {
      if (_name(ast.functionName) === 'main') {
        ...
        this.visitFunctionBody(ast.functionBody, level + 1, { main: true, argsName: args });
      } else {
  ...
  eachAPI(ast, level) {
    ...
    if (ast.runtimeBody) {
      this.emit(`_lastRequest = ${REQUEST_};\n`, baseLevel + 1);
    }

    this.emit(`TeaResponse ${RESPONSE_} = Tea.doAction(${REQUEST_}`, baseLevel + 1);
    if (ast.runtimeBody) {
      this.emit(`, ${RUNTIME_}`);
    } else {
  ...
  visitImport() {
    for (var item of this.usedExternModel.entries()) {
      ...
      let models = [...item[1]];
      let useless = 0;
      Object.keys(this.imports[item[0]].typedef || {}).forEach((key) => {
        if (models.includes(key)) {
          useless++;
        }
      });
      if (item[1].size - useless > 0) {
...
import com.aliyun.darabonba.*;
`);
    if (extendParam.writeExceptionImport) {
      this.emit('import com.aliyun.darabonba.exceptions.*;\n');
    }
    if (hasAPI) {
      this.emit('import com.aliyun.darabonba.interceptor.InterceptorChain;\n');
      this.emit('import com.aliyun.darabonba.interceptor.RuntimeOptionsInterceptor;\n');
      this.emit('import com.aliyun.darabonba.interceptor.RequestInterceptor;\n');
      this.emit('import com.aliyun.darabonba.interceptor.ResponseInterceptor;\n');
    }
...
public class ${this.className || 'Client'}`);
  ...
  exceptionBefore(level, exceptionName, extendOn) {
    if (this.editable !== true) {
      this.emit(`// This file is auto-generated, don't edit it. Thanks.\n`);
    }
    this.emit(`package ${this.package}.exceptions;\n\n`);
    this.emit(`import com.aliyun.darabonba.*;\n`);
    this.emit(`import java.util.Map;\n\n`);
    this.emit(`public class ${exceptionName} extends `);
  ...
  visitExtendOn(extendOn, type = 'model') {
    ...
    if (extendOn.type === 'moduleModel') {
      ...
      this.emit(`${this.imports[moduleId.lexeme].package}.exceptions.${modelName}`);
      ...
      if (extendOn.moduleName) {
        this.emit(`${this.imports[extendOn.moduleName].package}.exceptions.`);
      }
      this.emit(modelName);
    }
  }
  ...
  eachException(ast, level) {
    ...
    this.visitAnnotation(ast.annotation, level);
    this.visitExceptionBody(ast.exceptionBody, level + 1, exceptionName, ast.extendOn);
  }

  visitExceptionBody(ast, level, exceptionName, extendOn) {
    assert.equal(ast.type, 'exceptionBody');
    let node;
    for (let i = 0; i < ast.nodes.length; i++) {
      node = ast.nodes[i];
      const fieldName = _name(node.fieldName);
      // Skip fields that are already in parent exception (only if this exception extends another exception)
      const shouldSkip = extendOn && ['message', 'code', 'data', 'statusCode', 'description', 'accessDeniedDetail'].includes(fieldName);
      if (!shouldSkip) {
        let comments = DSL.comment.getFrontComments(this.comments, node.tokenRange[0]);
        this.visitComments(comments, level);
        this.emit('public ', level);
        this.visitFieldType(node.fieldValue, node, exceptionName);
        this.emit(` ${_name(node.fieldName)};\n`);
        this.emit('\n');
      }

      if (node.fieldValue.type === 'modelBody') {
        this.emit(`public static class ${exceptionName}${_upperFirst(node.fieldName.lexeme)} extends TeaModel {\n`, level);
        this.visitModelBody(node.fieldValue, level + 1, exceptionName + _upperFirst(node.fieldName.lexeme));
        this.emit('}\n', level);
      }
    }
    if (node) {
