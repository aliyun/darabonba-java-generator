...
class Visitor {
  ...
  visitType(ast, isSubType = false) {
    if (ast.type === 'map') {
      ...
      const checkModel = className + ':' + className.substring(index).substring(1);
      if (this.conflictModels.get(checkModel)) {
        this.emit(packageName + className.substring(index));
      } else {
        this.emit(className.substring(index).substring(1));
      }
  ...
  visitFieldType(value, node, modelName) {
    if (value.fieldType === 'array') {
      ...
      } else if (value.fieldItemType.type === 'map' || value.fieldItemType.fieldType === 'array') {
  ...
  createGetSetMethod(ast, level, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.emit(`public ${modelName} set`, level);
  ...
  eachFunction(ast, level) {
    ...
    if (_name(ast.functionName) === 'main') {
      this.emit(`(String[] ${_name(ast.params)}_)`);
    } else {
  ...
  visitRuntimeAfter(ast, level) {
    ...
    this.emit('continue;\n', level + 3);
    this.emit('}\n', level + 2);
    this.emit('throw new RuntimeException(e);\n', level + 2);
    this.emit('}\n', level + 1);
    this.emit('}\n\n', level);
